...
class Bc_Download_Helper_Download extends Mage_Core_Helper_Abstract
{
    const LINK_TYPE_URL         = 'url';
    const LINK_TYPE_FILE        = 'file';

    const XML_PATH_CONTENT_DISPOSITION  = 'catalog/download/content_disposition';
    ...
    protected function _getHandle()
    {
        if (!$this->_resourceFile) {
            Mage::throwException(Mage::helper('download')->__('Please set resource file and link type.'));
        }
        ...
        if (is_null($this->_handle)) {
            if ($this->_linkType == self::LINK_TYPE_URL) {
                ...
                if (!isset($urlProp['scheme'])
                    ...
                    Mage::throwException(Mage::helper('download')->__('Invalid download URL scheme.'));
                }
                if (!isset($urlProp['host'])) {
                    Mage::throwException(Mage::helper('download')->__('Invalid download URL host.'));
                }
                ...
                if ($this->_handle === false) {
                    Mage::throwException(Mage::helper('download')->__('Cannot connect to remote host, error: %s.', $errstr));
                }
                ...
                if (!isset($this->_urlHeaders['code']) || $this->_urlHeaders['code'] != 200) {
                    Mage::throwException(Mage::helper('download')->__('An error occurred while getting the requested content. Please contact the store owner.'));
                }
            }
            elseif ($this->_linkType == self::LINK_TYPE_FILE) {
                ...
                if (!$this->_handle->fileExists($this->_resourceFile, true)) {
                    Mage::throwException(Mage::helper('download')->__('The file does not exist.'));
                }
                $this->_handle->streamOpen($this->_resourceFile, 'r');
            }
            else {
                Mage::throwException(Mage::helper('download')->__('Invalid download link type.'));
            }
        }
        return $this->_handle;
    }
    ...
    public function getContentType()
    {
        ...
        if ($this->_linkType == self::LINK_TYPE_FILE) {
            if (function_exists('mime_content_type') && ($contentType = mime_content_type($this->_resourceFile))) {
                ...
                return Mage::helper('download/file')->getFileType($this->_resourceFile);
            }
        }
        elseif ($this->_linkType == self::LINK_TYPE_URL) {
     ...
     * @return Mage_download_Helper_Download
     */
